...
// Tried in order; the first one whose API key is set (and that returns a
// valid intent) wins. The deterministic rule parser is the final safety net.
  ...
  const devices = Array.isArray(obj.devices)
    ? obj.devices
        ...
        .map((d: any) => ({
          ...
          count: Math.max(1, Math.min(500, Number(d.count) || 1)),
        }))
    : [];
  const options = {
    vlan: Boolean(obj.options?.vlan),
    routingProtocol:
      typeof obj.options?.routingProtocol === "string"
        ? obj.options.routingProtocol
        : undefined,
  };
...
export async function parsePrompt(
  ...
  for (const provider of PROVIDERS) {
    ...
    try {
      ...
      console.warn(
        `[ai] provider "${provider.name}" failed, trying next:`,
        err,
      );
    }
  }
